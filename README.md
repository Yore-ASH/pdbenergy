...
> **别把"打标签"当成"训练"。** `ensemble` 是**造数据集**：对每个蛋白采样构象、用
> AMBER14+GBn2 隐式溶剂算真值能量，产物是 `data/interim/*.npz`，**全程不涉及神经网络**，
> 所以它再跑一次也不会让模型变准。真正的训练是 `train`（读 `data/interim/`，
> 写 `outputs/<model>_<split>/`）。想要"打标签 + 训练 + 评估 + 记录"一条龙，用 `iterate`。
...
底部那两个栏目（命令行预览 / 实时日志）都能放大：

| 操作 | 用途 |
|---|---|
| 拖两条界线 | 想给谁更多地方就拖谁；命令行预览不再有 52 px 的硬上限，命令再长也不会被切掉 |
| **Ctrl+E** / 工具栏「展开输出」 | 输出区整块占满窗口，再按一次还原（页面区有 320 px 的最小图片区，只调比例长不大，所以展开是整块让位）|
| **Ctrl+B** / 工具栏「隐藏侧栏」 | 收起左侧页面列表，把宽度让给内容 |

浏览器版（`pdbenergy web`）的日志默认只有 230 px，「展开日志」按钮或 **Ctrl+E** 让它铺满
整个窗口，**Esc** 还原。
...
数据目录一律**锚定在项目根目录**，与启动界面时的工作目录无关：相对默认值
（`data/raw`、`outputs`）会用项目根拼接成绝对路径。所以从桌面快捷方式、IDE 或别的
目录启动，界面看到的仍是同一份数据，不会出现"项目是空的"或者产物跑到别处。
...
### 三个实测结论（见 [`TeachFlow.md`](TeachFlow.md) §12.3）

项目里最容易踩的坑是**只看 MAE**。做了三个对照实验后：

| 改了什么 | val MAE | test MAE | test Spearman | 预测跨度 | 结论 |
|---|---|---|---|---|---|
| 出厂配置 | 129.53 | 121.39 | 0.430 | 2.4% | 基准 |
| **加数据** ×2 / ×4（81 → 160 → 323 构象） | 128.56 → **143.02** → 127.16 | — | — | — | ❌ **没用**，非单调，三点全在常数基线上方 |
| **加轮数** ×5（4→20 轮） | 129.53 → 136.01 | 121.39 → 121.06 | — | — | ❌ **没用**，4 轮就饱和 |
| **加密特征**（`n_rbf` 32→96，`rbf_end` 5.0→4.0） | **114.30** | **102.00** | **−0.036** | 2.0% | ⚠️ MAE 首次超过常数基线，**但排序能力塌了** |

（常数预测器：val 118.28 / test 107.29）

**怎么读**：加密特征把 MAE 降了 12–16%，训练损失却几乎没变（0.3158 → 0.3093）——
说明提升来自泛化而不是拟合。但 Spearman 从 0.430 掉到 −0.036，
预测跨度仍只有真实能量的 2%：**它是靠"把预测压得更接近均值"赢的 MAE，代价是丢掉排序信号。**

所以当前模型**还不能当打分函数**：它基本在输出一个略优于常数的常数。
下一步该动的是**目标设计**（`ΔE` 把"分子大小/热激发幅度"和"构象形变"混在了一起），
而不是继续加数据。详见 `TeachFlow.md` §12.3。

复现这些对照实验：

```powershell
python scripts\learning_curve.py --fractions 0.25 0.5 1.0 --epochs 8
python scripts\compare_configs.py --epochs 4 --variant base32_end5 `
    --variant rbf96_end4=features.n_rbf=96,features.rbf_end=4.0
python scripts\estimate_workload.py     # 这个数据集要标多少、多久
```
